...
- cPanel deployment: the blocker is the Python/FastAPI + Redis + Playwright-worker stack, not PostGIS (spatial code is ~7 files and dev already runs on SQLite). Don't contort the stack for shared hosting; use a small VPS with the existing docker-compose. The Laravel rewrite path was already rejected in the v2 plan.
- Rentals feature plan lives at `plans/valuadis-rentals/plan.mdx` (rent valuation + government rental registry, grounded in Proclamation 1320/2024 and the Addis 11.5% cap). Key settled decisions: rent valuations reuse `Valuation` with a `purpose` column; new `backend/app/modules/rentals/` module; no bidding (fixed band ±10% + applications), no deposit custody in v1 (receipt refs only); pilot = Addis Ababa Housing Administration, Bole + Yeka sub-cities. All open questions answered 2026-07-18 — plan is decision-complete, ready for Phase A.
- Rentals Phase B is implemented on `feat/rentals-phase-b`: `backend/app/modules/rentals/` at `/api/v1/rentals`, migrations to head `2026_07_20_0902_rental_roles`, public `/rent` pages, officer console `/rentals`, owner `/rentals/my-listings`. Citizen signup is `POST /api/v1/rentals/signup` (NOT `/auth/register` — that contract stays frozen); citizen license_number is derived as `FAYDA-<fayda_id>`.
- Public rentals payloads use a closed Pydantic serializer (`PublicListing`, `extra="forbid"`) so owner PII can never leak by omission. Keep this allowlist-by-construction pattern for any future public payloads (tenancy contracts especially).
- `GET /api/v1/rentals/listings` semantics: no `status` param = public published-only redacted search; any explicit `?status=` = rental_officer-only review queue with full detail. Passing `status=published` publicly is a 401 by design — do not "fix" it back into the public branch (that regression showed redacted rows in the officer console).
- New NOT NULL boolean columns on `users` (and similar hot tables) need `server_default=false()` in the ORM, not just `default=False`: existing tests insert users via raw SQL and hit NOT NULL violations otherwise.
- Regenerate the OpenAPI snapshot with `json.dump(spec, f, indent=1, sort_keys=True)` plus a trailing newline — that matches the committed format and yields a reviewable additive diff; `indent=2` rewrites all ~19k lines.
- Browser-proof sessions must drive `http://localhost:3000`, never `http://127.0.0.1:3000`: the httpOnly refresh cookie belongs to the API host `localhost:8020`, and a 127.0.0.1 page makes refresh cross-site (SameSite=lax), so every hard navigation bounces to /login.
- Rentals Phase C is implemented on `feat/rentals-phase-c`: `rental_applications` + `tenancy_contracts` + `rental_contract_sequences` (migrations to head `2026_07_21_0901_contracts`), band-enforced applications (422 outside band, 429 rate limit), officer-only contract registration with per-year `AA-RNT-<year>-<seq>` numbers, deposit-receipt-gated draft→active, and on-demand contract/listing-agreement PDFs marked "pilot draft — pending legal review".
- Contract numbers are allocated from `rental_contract_sequences` under `with_for_update()` with the unique constraint + retry as backstop; `tests/test_contract_sequence_concurrency.py` (20 parallel threads, contiguous sequence) is the regression proof. Do not replace this with random ids — contract numbers are quotable legal registry values.
- Alembic's `alembic_version.version_num` column is varchar(32); revision ids longer than 32 characters fail mid-upgrade on Postgres with StringDataRightTruncation. Keep revision ids short.
- Pydantic `EmailStr` rejects special-use TLDs like `.local`; use real-looking domains for dev/E2E accounts. HTML number inputs validate `step` relative to `min`, so band-constrained inputs must use `step="any"`.
- Rentals shipped: all 4 phases merged to main, tag `rentals-pilot-v1`; release evidence and Phase-2 scope in `plans/valuadis-rentals/PILOT-READINESS.md`. Final gates: 580 passed/17 skipped, E2E 10/10, +18 additive API operations, migration head `2026_07_22_0901_renewal_cap`.
- Review-1 fixes to preserve: citizen signup sets the httpOnly `valuadis_refresh` cookie and returns NO refresh token in the body; `users.fayda_id_number` has a DB unique index; listing creation is residential-only (`RENTABLE_PROPERTY_TYPES`) per Proclamation 1320/2024 — do not widen without a legal read.
- The renewal cap is read from `renewal_cap_configs` (region + effective period; seeder-managed) with a documented in-code fallback. Update the seeded row when the Addis directive changes the percentage — never edit the fallback constant as the "fix".
- Nuxt 3.21.5 route-name collision: `pages/rent/index/index.vue` silently collides with `pages/rent/index.vue` (same generated route name). A nested literal `index/` directory under a page that also has `index.vue` is a trap; use a flat file + `definePageMeta({ path })` instead.
- Phases E+F merged (main @ 227db0b, PR #68 updated): RBAC lives in `backend/app/core/rbac.py` + `tests/test_role_matrix.py` (the permission matrix in `plans/valuadis-rentals/tasks/phase-e.md` is the spec — keep new endpoints matrix-compliant); web personas derive from `frontend/app/utils/persona.ts` (staff|officer|owner|renter) and each has an allow-listed shell. The citizen mobile app is `mobile-rentals/` ("ValuAdis Rent", com.valuadis.rent) — separate from the valuer app `mobile/`; design contract in `mobile-rentals/DESIGN.md`.
- Parallel implementation agents in this repo MUST use isolated worktrees: two shared-checkout branch collisions occurred (an agent's commits landing on whatever branch was checked out). Direct pushes to main are hook-blocked; update PR #68 by force-moving `release/rentals-pilot-v1` and pushing that branch.
- Known non-blocking flags for the next pass: property `area_sqm` is recomputed from the boundary polygon (pre-existing, surfaces in contract PDFs), in-process rate limiters need a shared store under multi-worker deployment, application accept has a theoretical concurrent double-accept race (tighten with a row lock alongside renewals work).
- `/api/v1/audit/logs` raw SQL returns string timestamps on SQLite; the endpoint normalizes via hasattr('isoformat') now — keep normalization when touching audit reads.
- The rentals E2E chain (`frontend/tests/e2e/flows/rental-registry-workflow.spec.ts`) runs against an isolated SQLite backend (port 8060) + built preview (port 3060) with `PW_SKIP_WEBSERVER=1`; the bootstrap needs `ensure_development_sqlite_schema`, not plain `create_all` (geometry columns). An `is_admin` account satisfies both owner and officer role gates for test setup.
- The property create endpoint recomputes `area_sqm` from the boundary polygon, overriding the client-provided value; this surfaces in tenancy-contract PDFs (e.g. 180 m² became 12,159.89 m²). Pre-existing property-module behavior — flag for the final review pass, do not silently "fix" in rentals code.
- `PropertyMap.vue` marker clustering: `import('leaflet.markercluster')` has no constructor on `module.default` — use `L.MarkerClusterGroup` after the plugin patches `L`, and keep the try/finally that clears the loading overlay. This bug stranded the map spinner on every consumer page.
- Playwright MCP screenshots with relative filenames land in the repo root, and `*.png` is globally gitignored — move them into `plans/.../proof/<phase>/` and `git add -f`.

### 2026-07-19

- The `area_sqm` recompute bug flagged 2026-07-18 is fixed: `PropertyService.create_property`/`update_property` now treat an explicitly provided `area_sqm` as the source of truth and only fall back to the polygon-derived area when it's absent or zero. `PropertyCreate` requires `area_sqm`, so the live API/bulk-import path always supplies one; the fallback only matters for direct `PropertyService` callers that skip the schema.
- The application-accept race flagged 2026-07-18 is fixed with a row lock: `RentalListingRepository.get_locked()` (`with_for_update`) plus a post-lock `db.refresh(application)` recheck in `application_service._accept_with_lock`, mirroring `contract_service._next_sequence_value`. **SQLite caveat proven by testing**: `with_for_update()` is a silent no-op on SQLite, so a real two-thread race there still lets both accepts through — verified empirically (a `threading.Barrier` test failed 3/3 times against SQLite). The lock is correct and effective on Postgres (production); SQLite coverage is a monkeypatched "simulated interleave" test only, not true concurrency.
- Real property photo upload shipped: `property_photos` table (migration `2026_07_23_0900_property_photos`), `POST/GET/DELETE /api/v1/properties/{id}/photos[...]` (staff/owner write; read is public once the property backs a PUBLISHED rental listing, else owner/staff/officer-scoped), files on disk under `settings.MEDIA_ROOT` with server-generated filenames, re-validated via Pillow decode (not content-type). Public listing detail now carries `property.photo_urls`. This closes the "no-photo API gap" flagged in the mobile Phase F memory — the mobile app can wire up the same endpoints next.
- Found via a live Playwright console sweep (not caught by pytest): `SecurityHeadersMiddleware`'s global `Cross-Origin-Resource-Policy: same-origin` silently blocked the new photo-file endpoint from loading as `<img src>` on the frontend's different-port origin (`net::ERR_BLOCKED_BY_RESPONSE.NotSameOrigin`). Fixed with a path-regex exception for that one route only. Any future cross-origin-embeddable endpoint needs the same treatment — `TestClient` does not enforce this, only a real browser does.
- Dev-check ports 8000/8010/8020 are frequently occupied by *other, unrelated* local projects on this machine (docker `iam-service`, a stray PHP process, etc.) — check with `lsof -nP -iTCP:<port> -sTCP:LISTEN` before picking a port for a throwaway backend; 8030+ was free.
- Root `create_admin.py` is currently broken (`ImportError: cannot import name 'UserCreate' from app.modules.auth.schemas`) — not fixed in this session (out of scope). Use `POST /api/v1/auth/register` then flip `User.is_admin = True` via a one-off `SessionLocal()` script instead.
